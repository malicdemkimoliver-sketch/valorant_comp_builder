...
def render():
    ...
    if step == 1:
        ...
        with c1:
            if st.button("Next: Pick Agents →", type="primary", use_container_width=True, key="s1next"):
                st.session_state["builder_step"] = 2; st.rerun()
    ...
    # ══ STEP 3 — ANALYSIS ═════════════════════════════════════════════════════
        ...
        with cexp:
            ...
            agent_js  = json.dumps({a.name: AGENT_PORTRAITS.get(a.name,"") for a in s_obj})
            agent_dj  = json.dumps([{"name":a.name,"role":a.role,"color":RC.get(a.role,"#64748b"),"icon":a.icon} for a in s_obj])
            ...
            function doExport(){{
                const W=960, H=580;
                const agents={agent_dj}, imgs={agent_js};
                ...
                ctx.fillStyle=grad; ctx.fillRect(8,8,W-16,3);

                // Header
                ctx.fillStyle='#e2e8f0'; ctx.font='bold 22px Segoe UI,Arial,sans-serif';
                ctx.textAlign='left'; ctx.fillText("Gyd's Comp Helper", 28, 44);
                ctx.fillStyle='#00d4ff'; ctx.font='bold 22px Segoe UI,Arial,sans-serif';
                ctx.fillText(' — {selected_map}', 28+ctx.measureText("Gyd's Comp Helper").width, 44);

                // Score badge
                const scoreColors={{"S":"#ffd700","A":"#00ff9f","B":"#0ea5e9","C":"#f59e0b","D":"#ff4d6d"}};
                const gc=scoreColors['{gr}']||'#e2e8f0';
                ctx.fillStyle=gc+'33'; ctx.beginPath();
                ctx.roundRect(W-120,18,100,36,10); ctx.fill();
                ctx.strokeStyle=gc; ctx.lineWidth=1.5;
                ctx.beginPath(); ctx.roundRect(W-120,18,100,36,10); ctx.stroke();
                ctx.fillStyle=gc; ctx.font='bold 20px Segoe UI,Arial,sans-serif';
                ctx.textAlign='center'; ctx.fillText('{raw}/100  {gr}', W-70, 41);

                // Label
                ctx.fillStyle='#64748b'; ctx.font='13px Segoe UI,Arial,sans-serif';
                ctx.textAlign='left'; ctx.fillText('{lb}  ·  Map: {selected_map}', 28, 64);

                // Divider
                ctx.fillStyle='#1a2f4a'; ctx.fillRect(28,74,W-56,1);

                // Agent slots
                const slotW=(W-56)/agents.length; let done=0;
                const agentY=170;

                function renderText(){{
                    // ── Strengths column ──────────────────────────────────
                    const colW=(W-56)/3; const leftX=28;
                    ctx.fillStyle='#10b981'; ctx.font='bold 12px Segoe UI,Arial,sans-serif';
                    ctx.textAlign='left'; ctx.fillText('✅  STRENGTHS', leftX, agentY+110);
                    ctx.fillStyle='#1a2f4a'; ctx.fillRect(leftX, agentY+114, colW-10, 1);
                    pros.forEach((s,i)=>{{
                        ctx.fillStyle='#6ee7b7'; ctx.font='11px Segoe UI,Arial,sans-serif';
                        const txt='• '+s; const maxW=colW-16;
                        // simple word-wrap
                        const words=txt.split(' '); let line=''; let y=agentY+130+i*30;
                        words.forEach(w=>{{
                            const test=line+w+' ';
                            if(ctx.measureText(test).width>maxW&&line){{
                                ctx.fillText(line,leftX,y); line=w+' '; y+=14;
                            }}else{{line=test;}}
                        }});
                    ...
                    ctx.fillStyle='#ff4d6d'; ctx.font='bold 12px Segoe UI,Arial,sans-serif';
                    ctx.textAlign='left'; ctx.fillText('❌  WEAKNESSES', midX, agentY+110);
                    ctx.fillStyle='#1a2f4a'; ctx.fillRect(midX, agentY+114, colW-10, 1);
                    cons.forEach((s,i)=>{{
                        ctx.fillStyle='#fca5a5'; ctx.font='11px Segoe UI,Arial,sans-serif';
                        const txt='• '+s; const maxW=colW-16;
                        const words=txt.split(' '); let line=''; let y=agentY+130+i*30;
                        words.forEach(w=>{{
                            const test=line+w+' ';
                            if(ctx.measureText(test).width>maxW&&line){{
                                ctx.fillText(line,midX,y); line=w+' '; y+=14;
                            }}else{{line=test;}}
                        }});
                        ctx.fillText(line,midX,y);
                    }});

                    // ── Warnings / notes column ───────────────────────────
                    const rightX=leftX+colW*2;
                    ctx.fillStyle='#f59e0b'; ctx.font='bold 12px Segoe UI,Arial,sans-serif';
                    ctx.textAlign='left'; ctx.fillText('⚠️  WARNINGS', rightX, agentY+110);
                    ctx.fillStyle='#1a2f4a'; ctx.fillRect(rightX, agentY+114, colW-10, 1);
                    if(warns.length===0){{
                        ctx.fillStyle='#10b981'; ctx.font='11px Segoe UI,Arial,sans-serif';
                        ctx.fillText('✅ No warnings — solid comp!', rightX, agentY+132);
                    }}else{{
                        warns.forEach((s,i)=>{{
                            ctx.fillStyle='#fde68a'; ctx.font='11px Segoe UI,Arial,sans-serif';
                            const txt='• '+s; const maxW=colW-16;
                            const words=txt.split(' '); let line=''; let y=agentY+130+i*32;
                            words.forEach(w=>{{
                                const test=line+w+' ';
                                if(ctx.measureText(test).width>maxW&&line){{
                                    ctx.fillText(line,rightX,y); line=w+' '; y+=14;
                                }}else{{line=test;}}
                            }});
                    ...
                    const roleColors={{"Duelist":"#ff4d6d","Controller":"#7c3aed","Initiator":"#0ea5e9","Sentinel":"#10b981"}};
                    let rx=28; const ry=H-44;
                    roles.forEach(r=>{{
                        const rc=roleColors[r]||'#64748b';
                        const cnt=agents.filter(a=>a.role===r).length;
                        const label=cnt+' '+r+(cnt>1?'s':'');
                        const tw=ctx.measureText(label).width+20;
                        ctx.fillStyle=rc+'25'; ctx.beginPath();
                        ...
                        ctx.fillStyle=rc; ctx.font='bold 10px Segoe UI,Arial,sans-serif';
                        ctx.textAlign='center'; ctx.fillText(label,rx+tw/2,ry+13);
                        rx+=tw+8;
                    }});
                    ...
                    a2.download='comp_{selected_map.replace(" ","_")}.png';
                    a2.href=cv.toDataURL('image/png'); a2.click();
                }}

                agents.forEach((ag,i)=>{{
                    const cx=28+i*slotW+slotW/2;
                    const drawSlot=(imgEl)=>{{
                        // Role color ring background
                        ctx.fillStyle=ag.color+'18';
                        ctx.beginPath(); ctx.arc(cx,agentY,48,0,Math.PI*2); ctx.fill();

                        if(imgEl){{
                            ctx.save(); ctx.beginPath(); ctx.arc(cx,agentY,44,0,Math.PI*2); ctx.clip();
                            try{{ctx.drawImage(imgEl,cx-44,agentY-44,88,88);}}catch(e){{}}
                            ctx.restore();
                        }}else{{
                            ctx.fillStyle=ag.color; ctx.font='36px serif';
                            ctx.textAlign='center'; ctx.fillText(ag.icon||'?',cx,agentY+13);
                        }}
                        ctx.strokeStyle=ag.color; ctx.lineWidth=3;
                        ctx.beginPath(); ctx.arc(cx,agentY,46,0,Math.PI*2); ctx.stroke();

                        ctx.fillStyle='#e2e8f0'; ctx.font='bold 13px Segoe UI,Arial,sans-serif';
                        ctx.textAlign='center'; ctx.fillText(ag.name,cx,agentY+62);
                        ctx.fillStyle=ag.color; ctx.font='10px Segoe UI,Arial,sans-serif';
                        ctx.fillText(ag.role,cx,agentY+76);

                        if(++done===agents.length) renderText();
                    }};
                    const uri=imgs[ag.name]||'';
                    if(uri.startsWith('data:')){{
                        const im=new Image(); im.onload=()=>drawSlot(im);
                        im.onerror=()=>drawSlot(null); im.src=uri;
                    }}else{{ drawSlot(null); }}
                }});
            }}
            </script>""", height=50)
        ...
        with cb3:
            if st.button("🔄 Start Over",key="restart"): st.session_state["builder_step"]=1;st.session_state["selected_agents"]=[];st.rerun()
